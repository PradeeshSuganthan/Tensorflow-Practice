...
x = tf.random_normal([300], mean = 10.0, stddev = 1.0)
y = tf.random_normal([300], mean = 10.0, stddev = 1.0)

average = tf.reduce_mean(x-y)
cond = tf.less(average,0)
meansquare = tf.reduce_mean(tf.square(x-y))
abssum = tf.reduce_sum(tf.abs(x-y))

out = tf.where(cond, meansquare, abssum)